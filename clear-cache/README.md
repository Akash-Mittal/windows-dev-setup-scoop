...
## `start-clear-cache.ps1`

The `start-clear-cache.ps1` script removes temporary files and caches from the following locations:
...
1.  **Via `fire.ps1`**:
    The `fire.ps1` script (located in the repository root) is configured to call `start-clear-cache.ps1` as part of its startup sequence.
